package ALGORYTMY.Structures.ZADANIA_C;
...
//        mergeSort(0,table.length-1);  // sortowanie przez scalanie (mniejszych posortowanych juz czesci)
    ...
    {
        for (int i = 0; i < table.length - 1; i++)   // petla zewnetrzna (malejaca)
        {
            for (int j = 0; j < table.length -1 -i; j++)        // petla wewnetrzna (rosnaca)
            {
